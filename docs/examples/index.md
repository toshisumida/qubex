# Examples

This section links to the rendered Jupyter notebooks shipped with the documentation.

If you are new to Qubex, start with [Choose where to start](../user-guide/getting-started/choose-where-to-start.md) and then continue with the corresponding guide:

- [`Experiment`](../user-guide/experiment/index.md)
- [`QuantumSimulator`](../user-guide/simulator/index.md)
- [Low-level APIs](../user-guide/low-level-apis/index.md)

## Experiment

- [Basic usage](experiment/0_basic_usage.ipynb)
- [Spectroscopy](experiment/1_spectroscopy.ipynb)
- [Frequency calibration](experiment/2_frequency_calibration.ipynb)
- [Pulse calibration](experiment/3_pulse_calibration.ipynb)
- [T1 and T2 experiments](experiment/4_t1_t2_experiments.ipynb)
- [Randomized benchmarking](experiment/5_randomized_benchmarking.ipynb)
- [State tomography](experiment/6_state_tomography.ipynb)
- [EF characterization](experiment/7_ef_characterization.ipynb)
- [State classification](experiment/8_state_classification.ipynb)
- [CR calibration](experiment/9_cr_calibration.ipynb)

## Low-level APIs

### `measurement`

- [Measurement config](measurement/measurement_config.ipynb)
- [Measurement session](measurement/measurement_client.ipynb)
- [Loopback capture](measurement/capture_loopback.ipynb)
- [DSP classification](measurement/mid_circuit_dsp_classification.ipynb)
- [Measurement sweep builder](measurement/sweep_measurement_builder.ipynb)
- [Sweep measurement executor](measurement/sweep_measurement_executor.ipynb)

### `system`

- [ConfigLoader Example](system/config_loader.ipynb)
- [QuEL-1 skew adjustment workflow](system/quel1_skew_adjustment.md)
- [QuEL-3 Experiment Configure Check](system/quel3_experiment_configure_check.ipynb)

### `backend`

- [QuEL-1 continuous-wave output](backend/quel1_continuous_wave.ipynb)
- [PulseSchedule to QuEL-3 Sequencer Flow](measurement/quel3_sequencer_builder_flow.ipynb)
- [QuEL-3 Deploy Check](system/quel3_deploy_check.ipynb)

## QuantumSimulator

- [Rabi oscillation](simulator/1_rabi_oscillation.ipynb)
- [Two coupled qubits](simulator/2_coupled_qubits.ipynb)
- [Qubit resonator](simulator/3_qubit_resonator.ipynb)
- [Multi control](simulator/4_multi_control.ipynb)
- [Pi pulse](simulator/5_pi_pulse.ipynb)
- [DRAG calibration](simulator/6_drag_calibration.ipynb)
- [CR dynamics](simulator/7_cr_dynamics.ipynb)
- [CR calibration](simulator/8_cr_calibration.ipynb)
- [Jazz](simulator/9_jazz.ipynb)

## Pulse

- [Build pulse sequences with PulseSchedule](../user-guide/pulse-sequences/index.md)
- [Pulse tutorial](pulse/tutorial.ipynb)
- [Shape hash and waveform reuse](pulse/shape_hash_and_waveform_reuse.ipynb)

## Analysis

- [Fitting](analysis/fitting.ipynb)
- [Plotting](analysis/plot.ipynb)
- [Rotation 3D](analysis/rotation3d.ipynb)

## Core

- [Async bridge](core/async_bridge.ipynb)
- [Data model](core/data_model.ipynb)
- [Expression](core/expression.ipynb)
- [Model](core/model.ipynb)

## Clifford

- [Clifford usage](clifford/usage.ipynb)
- [Clifford mapping](clifford/mapping.ipynb)
